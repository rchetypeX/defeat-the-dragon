...
export function GameDashboard() {
  console.log('🎮 GameDashboard component is rendering!');
  console.log('🔥 FOREST BACKGROUND TEST - COMPONENT UPDATED!');
  ...
  const handleSessionStart = async (action: Action, durationMinutes: number) => {
    ...
    try {
      ...
      setShowSessionTimer(false); // Hide the timer modal when session starts
    } catch (error) {
    ...
    <div className="min-h-screen relative overflow-hidden">
      ...
      <div className="absolute inset-0 z-0">
        <img 
          ...
          onError={(e) => {
            console.error('Forest background failed to load, using fallback');
        ...
        />
        <div className="absolute inset-0 bg-gradient-to-b from-[#0f1419] via-[#1a2e1a] to-[#2e1a1a] opacity-60 hidden"></div>
      </div>
          ...
          {sessionProgress.isActive ? (
            ...
            <div className="text-center">
              ...
              <div className="w-full max-w-xs sm:max-w-sm mx-auto">
                ...
                <button
                  onClick={() => setShowSessionTimer(true)}
                  className="w-full max-w-xs sm:max-w-sm mx-auto bg-[#f5f5dc] border-4 border-[#8B4513] text-[#8B4513] font-bold text-lg sm:text-xl py-3 sm:py-4 px-6 sm:px-8 hover:bg-[#e8e8d0] transition-colors shadow-lg hidden"
                >
                  FOCUS
                </button>
              </div>
